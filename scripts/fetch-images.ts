...
/**
 * Images the restaurant owns. Not fetched and not credited — they are listed
 * here so regenerating the manifest cannot drop them, which is exactly what
 * would happen if they were only ever hand-added to the generated file.
 */
const OWNED: Slot[] = [
  {
    id: "about-ronika",
    slug: "ronika-portrait",
...
function renderManifest(
  ...
  const allSlugs = [...SOURCE_FILES.map((f) => f.slug), ...OWNED.map((o) => o.slug)];
  const imports = allSlugs
    .map((slug) => `import ${toIdentifier(slug)} from "@/assets/images/${slug}.jpg";`)
    .join("\n");
  ...
  const ownedEntries = OWNED.map((slot) =>
    [
      `  "${slot.id}": {`,
      `    image: ${toIdentifier(slot.slug)},`,
      `    alt: ${JSON.stringify(slot.alt)},`,
...
async function main(): Promise<void> {
  ...
  console.log(
    `${SLOTS.length} licensed slots across ${SOURCE_FILES.length} photographs, ` +
      `plus ${OWNED.length} owned. The licensed ones require attribution — ` +
      `/credits must stay published.`,
  );
}
